...
from src.datamodules.constants import MSG_WAVELENGTHS
...
def _process_msg_datatree(
    ...
    with xr.open_datatree(file, decode_timedelta=True) as goes_datatree:
        ...
        if "data" in ds.data_vars:
            ...
            data_dict["data"] = ds[list(MSG_WAVELENGTHS.keys())].to_array().values.astype(np.float32)
...
class CloudsatMSGDataset(Dataset):
    """
    Class to load MSG and CloudSat data.
    """
    ...
    def __getitem__(self, ind):  # can output array or dict depending on transforms
        ...
        while attempts_remaining > 0:
            try:
                ...
                data_dict = _process_msg_datatree(
                    file=file,
        ...
        data_dict["satellite"] = "msg_cloudsat"
