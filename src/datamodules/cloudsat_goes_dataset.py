...
from src.datamodules.constants import GOES_WAVELENGTHS
from src.datamodules.goes_dataset import scale_reflectance
...
def _process_goes_datatree(
    ...
    with xr.open_datatree(file, decode_timedelta=True) as goes_datatree:
        ...
        if "data" in ds.data_vars:
            ...
            data_dict["data"] = ds[list(GOES_WAVELENGTHS.keys())].to_array().values.astype(np.float32)
...
class CloudsatGOESDataset(Dataset):
    """
    ...
    def __getitem__(self, ind):  # can output array or dict depending on transforms
        ...
        while attempts_remaining > 0:
            try:
                ...
                data_dict = _process_goes_datatree(
                    file=file,
        ...
        data_dict["satellite"] = "goes_cloudsat"
