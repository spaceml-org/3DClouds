from __future__ import annotations

import math
from functools import partial
from typing import Callable

import torch
import torch.nn as nn

# vision_transformer requires torchvision >= 0.12
from . import utils, vision_transformer
from .vision_transformer import ConvStemConfig
from src.models.unet_utils.utils import UnetDecoder, UnetEncoder

class MaskedAutoEncoderEncoder(vision_transformer.Encoder):
    """Encoder for the Masked Autoencoder model [0].

    Encodes patch embeddings. Code inspired by [1].

    - [0]: Masked Autoencoder, 2021, https://arxiv.org/abs/2111.06377
    - [1]: https://github.com/facebookresearch/mae

    Attributes:
        seq_length:
            Token sequence length, including the class token.
        num_layers:
            Number of transformer blocks.
        num_heads:
            Number of attention heads.
        hidden_dim:
            Dimension of the input and output tokens.
        mlp_dim:
            Dimension of the MLP in the transformer block.
        dropout:
            Percentage of elements set to zero after the MLP in the transformer.
        attention_dropout:
            Percentage of elements set to zero after the attention head.

    """

    def __init__(
        self,
        seq_length: int,
        num_layers: int,
        num_heads: int,
        hidden_dim: int,
        mlp_dim: int,
        dropout: float,
        attention_dropout: float,
        norm_layer: Callable[..., torch.nn.Module] = partial(nn.LayerNorm, eps=1e-6),
    ):
        """ Initialize MaskedAutoEncoderEncoder.

            Parameters
            ----------
            seq_length : int. Token sequence length, including the class token.
            num_layers : int. Number of transformer blocks.
            num_heads : int. Number of attention heads.
            hidden_dim : int. Dimension of the input and output tokens.
            mlp_dim : int. Dimension of the MLP in the transformer block.
            dropout : float. Dropout rate applied after the MLP.
            attention_dropout : float. Dropout rate applied after the attention head.
            norm_layer : Callable. Callable that creates a normalization layer (optional).

            Returns
            -------
            None.
        """
        super().__init__(

    @classmethod
    def from_vit_encoder(
        cls, vit_encoder: vision_transformer.Encoder
    ) -> MaskedAutoEncoderEncoder:
        """ Create a MaskedAutoEncoderEncoder from a torchvision ViT encoder.

            Parameters
            ----------
            vit_encoder : vision_transformer.Encoder. Source ViT encoder whose weights are copied.

            Returns
            -------
            MaskedAutoEncoderEncoder. New encoder with weights copied from vit_encoder.
        """
        # Create a new instance with dummy values as they will be overwritten
        # by the copied vit_encoder attributes
        encoder = cls(
            seq_length=1,
            num_layers=1,
            num_heads=1,
            hidden_dim=1,
            mlp_dim=1,
            dropout=0,
            attention_dropout=0,
        )
        encoder.pos_embedding = vit_encoder.pos_embedding
        encoder.dropout = vit_encoder.dropout
        encoder.layers = vit_encoder.layers
        encoder.ln = vit_encoder.ln
        return encoder

    def forward(
        self, input: torch.Tensor, idx_keep: torch.Tensor | None = None
    ) -> torch.Tensor:
        """ Encode input tokens.

            Parameters
            ----------
            input : torch.Tensor. Batch of token sequences.
            idx_keep : torch.Tensor | None. Tensor with shape (batch_size, num_tokens_to_keep) where
                each entry is an index of the token to keep. If specified, only those tokens are encoded (optional).

            Returns
            -------
            torch.Tensor. Batch of encoded output tokens.
        """
        input = input + self.interpolate_pos_encoding(
            input
        )  # ========= These lines replace the forward pass of vit encoder
        # shape = [batch_size, (img_size / patch_size)**2 +1, hidden_dim]
        if (
            idx_keep is not None
        ):  # Original implementation does not interpolate positional encoding
            input = utils.get_at_index(input, idx_keep)
        # pass through encoder blocks, including layer normalization
        return self.ln(
            self.layers(self.dropout(input))
        )  # shape = [batch_size, (img_size / patch_size)**2 +1, hidden_dim]

    def interpolate_pos_encoding(self, input: torch.Tensor):
        """ Return the interpolated positional embedding for the given input.

            This function interpolates self.pos_embedding for all tokens in the input,
            ignoring the class token. This allows encoding variable sized images.

            Parameters
            ----------
            input : torch.Tensor. Input tensor with shape (batch_size, num_sequences, hidden_dim).

            Returns
            -------
            torch.Tensor. Interpolated positional embedding with shape (1, num_sequences, hidden_dim).
        """
        # code copied from:
        # https://github.com/facebookresearch/msn/blob/4388dc1eadbe3042b85d3296d41b9b207656e043/src/deit.py#L291
        npatch = input.shape[1] - 1
        N = self.pos_embedding.shape[1] - 1  # shape = [(img_size / patch_size)**2 -1]
        if npatch == N:
            return self.pos_embedding
        class_emb = self.pos_embedding[:, 0]  # shape = [1, 1, hidden_dim]
        pos_embedding = self.pos_embedding[:, 1:]  # shape = [hidden_dim]
        dim = input.shape[-1]  # shape = [hidden_dim]
        pos_embedding = nn.functional.interpolate(
            pos_embedding.reshape(1, int(math.sqrt(N)), int(math.sqrt(N)), dim).permute(
                0, 3, 1, 2
            ),
            scale_factor=math.sqrt(npatch / N),
            mode="bicubic",
        )
        pos_embedding = pos_embedding.permute(0, 2, 3, 1).view(1, -1, dim)
        return torch.cat((class_emb.unsqueeze(0), pos_embedding), dim=1)


class MaskedAutoEncoderBackbone(vision_transformer.VisionTransformer):
    """Backbone for the Masked Autoencoder model [0].

    Converts images into patches and encodes them. Code inspired by [1].
    Note that this implementation uses a learned positional embedding while [0]
    uses a fixed positional embedding.

    - [0]: Masked Autoencoder, 2021, https://arxiv.org/abs/2111.06377
    - [1]: https://github.com/facebookresearch/mae
    - [2]: Early Convolutions Help Transformers See Better, 2021, https://arxiv.org/abs/2106.14881.

    Attributes:
        image_size:
            Input image size.
        patch_size:
            Width and height of the image patches. image_size must be a multiple
            of patch_size.
        num_layers:
            Number of transformer blocks.
        num_heads:
            Number of attention heads.
        hidden_dim:
            Dimension of the input and output tokens.
        mlp_dim:
            Dimension of the MLP in the transformer block.
        dropout:
            Percentage of elements set to zero after the MLP in the transformer.
        attention_dropout:
            Percentage of elements set to zero after the attention head.
        num_classes:
            Number of classes for the classification head. Currently not used.
        representation_size:
            If specified, an additional linear layer is added before the
            classification head to change the token dimension from hidden_dim
            to representation_size. Currently not used.
        norm_layer:
            Callable that creates a normalization layer.
        conv_stem_configs:
            If specified, a convolutional stem is added at the beginning of the
            network following [2]. Not used in the original Masked Autoencoder
            paper [0].

    """

    def __init__(
        self,
        image_size: int,
        patch_size: int,
        num_layers: int,
        num_heads: int,
        hidden_dim: int,
        mlp_dim: int,
        dropout: float = 0,
        attention_dropout: float = 0,
        num_classes: int = 1000,
        representation_size: int | None = None,
        norm_layer: Callable[..., torch.nn.Module] = partial(nn.LayerNorm, eps=1e-6),
        conv_stem_configs: list[ConvStemConfig] | None = None,
    ):
        """ Initialize MaskedAutoEncoderBackbone.

            Parameters
            ----------
            image_size : int. Input image size; must be divisible by patch_size.
            patch_size : int. Size of each image patch in pixels.
            num_layers : int. Number of transformer encoder blocks.
            num_heads : int. Number of attention heads per block.
            hidden_dim : int. Dimension of the token embeddings.
            mlp_dim : int. Dimension of the MLP inside each transformer block.
            dropout : float. Dropout rate applied after the MLP (optional).
            attention_dropout : float. Dropout rate applied after attention (optional).
            num_classes : int. Number of output classes (optional).
            representation_size : int | None. If set, adds a pre-logits linear layer (optional).
            norm_layer : Callable. Callable that returns a normalization layer (optional).
            conv_stem_configs : list[ConvStemConfig] | None. If set, uses a convolutional stem (optional).

            Returns
            -------
            None.
        """
        super().__init__(
            image_size=image_size,
            patch_size=patch_size,
            num_layers=num_layers,
            num_heads=num_heads,
            hidden_dim=hidden_dim,
            mlp_dim=mlp_dim,
            dropout=dropout,
            attention_dropout=attention_dropout,
            num_classes=num_classes,
            representation_size=representation_size,
            norm_layer=norm_layer,
            conv_stem_configs=conv_stem_configs,
        )
        self.encoder = MaskedAutoEncoderEncoder(
            seq_length=self.seq_length,
            num_layers=num_layers,
            num_heads=num_heads,
            hidden_dim=hidden_dim,
            mlp_dim=mlp_dim,
            dropout=dropout,
            attention_dropout=attention_dropout,
            norm_layer=norm_layer,
        )


    @classmethod
    def from_vit(
        cls, vit: vision_transformer.VisionTransformer, unet_tokenize: bool=False, filters_seq=[],
    ) -> MaskedAutoEncoderBackbone:
        """ Create a MaskedAutoEncoderBackbone from a torchvision ViT model.

            Parameters
            ----------
            vit : vision_transformer.VisionTransformer. Source ViT model whose weights are copied.
            unet_tokenize : bool. If True, replaces the patch projection with a UNet encoder (optional).
            filters_seq : list. Filter sizes for the UNet encoder/decoder (optional).

            Returns
            -------
            MaskedAutoEncoderBackbone. New backbone with weights copied from vit.
        """
        # Create a new instance with dummy values as they will be overwritten
        # by the copied vit_encoder attributes
        backbone = cls(
            image_size=vit.image_size,
            patch_size=vit.patch_size,
            num_layers=1,
            num_heads=1,
            hidden_dim=vit.hidden_dim,
            mlp_dim=vit.mlp_dim,
            dropout=vit.dropout,
            attention_dropout=vit.attention_dropout,
            num_classes=vit.num_classes,
            representation_size=vit.representation_size,
            norm_layer=vit.norm_layer,
        )
        backbone.conv_proj = vit.conv_proj
        backbone.class_token = vit.class_token
        backbone.seq_length = vit.seq_length
        backbone.num_channels = vit.num_channels
        backbone.heads = vit.heads
        backbone.unet_tokenize = unet_tokenize
        if backbone.unet_tokenize:
            backbone.filters_seq = filters_seq
            backbone.unet_encoder = UnetEncoder(backbone.num_channels, backbone.hidden_dim, backbone.filters_seq)

        backbone.encoder = MaskedAutoEncoderEncoder.from_vit_encoder(vit.encoder)

        return backbone

    def images_to_tokens(
        self, images: torch.Tensor, unet_tokenize: bool, prepend_class_token: bool
    ) -> torch.Tensor:
        """ Convert images into patch tokens.

            Parameters
            ----------
            images : torch.Tensor. Tensor with shape (batch_size, channels, image_size, image_size).
            unet_tokenize : bool. If True, uses the UNet encoder instead of the patch projection.
            prepend_class_token : bool. If True, prepends the class token to the sequence.

            Returns
            -------
            torch.Tensor. Token tensor with shape (batch_size, sequence_length, hidden_dim)
            or (batch_size, sequence_length + 1, hidden_dim) if prepend_class_token.
        """
        # image.shape = [batch_size, n_channels, img_size, img_size]
        # either use a unet or a convolution with stride=patch_size for tokenizing the image
        if unet_tokenize:
            x = self.unet_encoder(images)
        else:
            x = self.conv_proj(images)
        # x.shape = [batch_size, hidden_dim, img_size/patch_size, img_size/patch_size]
        tokens = x.flatten(2).transpose(1, 2)
        # tokens.shape = [batch_size, (img_size/patch_size)**2, hidden_dim]
        if prepend_class_token:
            tokens = utils.prepend_class_token(tokens, self.class_token)
        return tokens

    def forward(
        self, images: torch.Tensor, idx_keep: torch.Tensor | None = None
    ) -> torch.Tensor:
        """ Return encoded class tokens from a batch of images.

            Parameters
            ----------
            images : torch.Tensor. Tensor with shape (batch_size, channels, image_size, image_size).
            idx_keep : torch.Tensor | None. Tensor with shape (batch_size, num_tokens_to_keep);
                if specified, only those tokens are passed to the encoder (optional).

            Returns
            -------
            torch.Tensor. Tensor with shape (batch_size, hidden_dim) containing the
            encoded class token for every image.
        """
        out = self.encode(images, idx_keep)
        class_token = out[:, 0]
        return class_token

    def encode(
        self, images: torch.Tensor, idx_keep: torch.Tensor | None = None
    ) -> torch.Tensor:
        """ Return encoded class and patch tokens from images.

            Parameters
            ----------
            images : torch.Tensor. Tensor with shape (batch_size, channels, image_size, image_size).
            idx_keep : torch.Tensor | None. Tensor with shape (batch_size, num_tokens_to_keep);
                if specified, only those tokens are passed to the encoder (optional).

            Returns
            -------
            torch.Tensor. Tensor with shape (batch_size, sequence_length, hidden_dim)
            containing the encoded class and patch tokens for every image.
        """
        # image.shape = [batch_size, n_channels, img_size, img_size]
        out = self.images_to_tokens(  # Linear projection
            images,self.unet_tokenize, prepend_class_token=True
        )  # shape = (batch_size, (img_size/patch_size)**2 +1, hidden_dim)
        return self.encoder(out, idx_keep)  # Filter by idx_keep, pass thru vit


class MaskedAutoEncoderDecoder(vision_transformer.Encoder):
    """Decoder for the Masked Autoencoder model [0].

    Decodes encoded patches and predicts pixel values for every patch.
    Code inspired by [1].

    - [0]: Masked Autoencoder, 2021, https://arxiv.org/abs/2111.06377
    - [1]: https://github.com/facebookresearch/mae

    Attributes:
        seq_length:
            Token sequence length, including the class token.
        num_layers:
            Number of transformer blocks.
        num_heads:
            Number of attention heads.
        embed_input_dim:
            Dimension of the input tokens. Usually be equal to the hidden
            dimension of the MaskedAutoEncoderEncoder or MaskedAutoEncoderBackbone.
        hidden_dim:
            Dimension of the decoder tokens.
        mlp_dim:
            Dimension of the MLP in the transformer block.
        out_dim:
            Output dimension of the prediction for a single patch. Usually equal
            to (3 * patch_size ** 2).
        dropout:
            Percentage of elements set to zero after the MLP in the transformer.
        attention_dropout:
            Percentage of elements set to zero after the attention head.

    """

    def __init__(
        self,
        seq_length: int,
        num_layers: int,
        num_heads: int,
        embed_input_dim: int,
        hidden_dim: int,
        mlp_dim: int,
        out_dim: int,
        dropout: float = 0.0,
        attention_dropout: float = 0.0,
        norm_layer: Callable[..., nn.Module] = partial(nn.LayerNorm, eps=1e-6),
        unet_tokenize: bool=False,
        filters_seq=[],
        num_channels=11,
    ):
        """ Initialize MaskedAutoEncoderDecoder.

            Parameters
            ----------
            seq_length : int. Token sequence length, including the class token.
            num_layers : int. Number of transformer decoder blocks.
            num_heads : int. Number of attention heads.
            embed_input_dim : int. Dimension of the input tokens from the encoder.
            hidden_dim : int. Internal decoder token dimension.
            mlp_dim : int. Dimension of the MLP in each transformer block.
            out_dim : int. Output dimension per patch (e.g. channels * patch_size**2).
            dropout : float. Dropout rate applied after the MLP (optional).
            attention_dropout : float. Dropout rate applied after attention (optional).
            norm_layer : Callable. Callable that returns a normalization layer (optional).
            unet_tokenize : bool. If True, uses a UNet decoder head instead of a linear layer (optional).
            filters_seq : list. Filter sizes for the UNet decoder (optional).
            num_channels : int. Number of output image channels (optional).

            Returns
            -------
            None.
        """
        super().__init__(
            seq_length=seq_length,
            num_layers=num_layers,
            num_heads=num_heads,
            hidden_dim=hidden_dim,
            mlp_dim=mlp_dim,
            dropout=dropout,
            attention_dropout=attention_dropout,
            norm_layer=norm_layer,
        )
        self.decoder_embed = nn.Linear(embed_input_dim, hidden_dim, bias=True)


        self.unet_tokenize = unet_tokenize
        self.num_channels = num_channels
        if self.unet_tokenize:
            self.filters_seq = filters_seq[::-1]
            self.unet_decoder = UnetDecoder(self.num_channels, hidden_dim, self.filters_seq)

        self.prediction_head = nn.Linear(hidden_dim, out_dim)

    def forward(self, input: torch.Tensor) -> torch.Tensor:
        """ Return predicted pixel values from encoded tokens.

            Parameters
            ----------
            input : torch.Tensor. Tensor with shape (batch_size, seq_length, embed_input_dim).

            Returns
            -------
            torch.Tensor. Tensor with shape (batch_size, seq_length, out_dim).
        """
        out = self.embed(input)
        out = self.decode(out)
        return self.predict(out)

    def embed(self, input: torch.Tensor) -> torch.Tensor:
        """ Embed encoded input tokens into the decoder token dimension.

            This is a single linear layer that projects from embed_input_dim to hidden_dim.

            Parameters
            ----------
            input : torch.Tensor. Tensor with shape (batch_size, seq_length, embed_input_dim).

            Returns
            -------
            torch.Tensor. Tensor with shape (batch_size, seq_length, hidden_dim).
        """
        return self.decoder_embed(input)

    def decode(self, input: torch.Tensor) -> torch.Tensor:
        """ Run a forward pass through the decoder transformer.

            Parameters
            ----------
            input : torch.Tensor. Tensor with shape (batch_size, seq_length, hidden_dim).

            Returns
            -------
            torch.Tensor. Tensor with shape (batch_size, seq_length, hidden_dim).
        """
        return super().forward(input)

    def decode_layers(self, input: torch.Tensor) -> torch.Tensor:
        """ Run a forward pass through the decoder transformer layers.

            Assumes that positional encoding has already been added to the input.

            Parameters
            ----------
            input : torch.Tensor. Tensor with shape (batch_size, seq_length, hidden_dim).

            Returns
            -------
            torch.Tensor. Tensor with shape (batch_size, seq_length, hidden_dim).
        """
        return self.ln(self.layers(self.dropout(input)))

    def predict(self, input: torch.Tensor) -> torch.Tensor:
        """ Predict pixel values from decoded tokens.

            Parameters
            ----------
            input : torch.Tensor. Tensor with shape (batch_size, seq_length, hidden_dim).

            Returns
            -------
            torch.Tensor. Tensor with shape (batch_size, seq_length, out_dim).
        """
        return self.prediction_head(input)

